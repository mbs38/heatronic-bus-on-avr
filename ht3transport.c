#include "ht3transport.h"

#define waitForSync 0
#define waitForFrame 1
#define receive 2
volatile uint8_t recstate = 0;

#define noData 0
#define dataValid 1
volatile uint8_t datastate = 0;
volatile uint8_t bufpos = 0;
volatile uint8_t buffer[64];

const PROGMEM uint8_t crc_table[] = {
	
};

uint8_t ht3CheckCRC(volatile uint8_t * buffer, volatile uint8_t bufpos) {
	uint8_t crc = 0;
        if (bufpos < 2) {
            return 0;
	}
        for (uint8_t i = 0; i < (bufpos - 1); i++) {
        	crc = pgm_read_byte(&crc_table[crc]);
                crc ^= buffer[i];
	}
        
	if (crc == buffer[bufpos-1])
        	return 1;
        else 
                return 0;
}



void ht3Init(void)
{
	UBRRH = (unsigned char)((UBRR) >> 8);
	UBRRL = (unsigned char) UBRR;
	UART_STATUS = (1<<U2X); //double speed mode.
   	UCSRC = (3<<UCSZ0); //Frame Size
	UART_CONTROL = (1<<RXCIE)|(1<<RXEN); // USART receiver
}

ISR(UART_RECEIVE_INTERRUPT)
{
	if (UART_STATUS & (1<<4)) //Frame Error
	{
		uint8_t data = UDR0;
		if (data==0x00) { //EOF
			switch(recstate) {
				case waitForSync: {
					recstate=waitForFrame;
				}
				break;

				case receive: { //EOF
					datastate=dataValid;
				}
				break;
			}
		} else { //some other error
			recstate=waitForSync;
		}
	}
	else { //no error
		uint8_t data = UDR0;
		if (data==0x88) { //Start of telegram
			if (datastate==noData) {
				bufpos = 0;
				buffer[bufpos]=data;
				bufpos++;
				recstate = receive;
			}
		} else {
			if (datastate==noData) {
				buffer[bufpos]=data;
				bufpos++;
			}
		}
	}
}

